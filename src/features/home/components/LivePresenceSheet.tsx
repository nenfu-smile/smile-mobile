...
function Chip({
  ...
  return (
    <Pressable
      ...
      <Text className={cn("text-sm font-medium", selected ? "text-white" : "text-neutral-900")}>
        {label}
      </Text>
    </Pressable>
  );
...
export function LivePresenceSheet({ visible, onClose }: LivePresenceSheetProps) {
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      ...
      <SafeAreaView edges={["bottom"]} className="max-h-[85%] rounded-t-[40px] bg-white">
        ...
        <ScrollView className="px-6" showsVerticalScrollIndicator={false}>
          ...
          <Text className="mb-2 mt-6 text-base font-semibold text-neutral-900">2. Select Mood</Text>
          <View className="flex-row flex-wrap gap-2">
            {MOODS.map((item) => (
              <Chip
                key={item.key}
              ...
              <Pressable
                ...
                <View
                  className={cn(
                    ...
                    visibility === key ? "bg-primary" : "border border-neutral-300",
                  )}>
                  {visibility === key ? <Text className="text-xs text-white">✓</Text> : null}
                </View>
              </Pressable>
            ))}
          </View>
