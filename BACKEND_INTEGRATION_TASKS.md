...
- [ ] **Auth method.** The current signup flow collects both an email (with an OTP-style "verify email" step) *and* a phone number (with its own "verify phone" step) before profile info. Is the real auth contract email+password with phone as a secondary verified field, phone-first OTP auth with email optional, or something else? This decides the whole auth module shape.
